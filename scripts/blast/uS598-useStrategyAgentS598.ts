...
let strategyAgent598ID = 598;
let strategyAgent598Address = "0xE75273Dad204c1F40f08c59Ce5B24294BeFBf8fe";
let strategyAgent598: BlastooorStrategyAgentAccount;
let agentModule598: LoopooorModuleD;
...
async function main() {
  ...
  strategyAgent598 = await ethers.getContractAt("BlastooorStrategyAgentAccountV2", strategyAgent598Address, boombotseth) as BlastooorStrategyAgentAccountV2;
  agentModule598 = await ethers.getContractAt("LoopooorModuleD", strategyAgent598Address, boombotseth) as LoopooorModuleD;
...
async function checkOwner() {
  let owner1 = await strategyCollection.ownerOf(strategyAgent598ID)
  console.log(`nft owner   : ${owner1}`)
  let owner2 = await strategyAgent598.owner()
...
async function withdrawBalances() {
  ...
  for(let token of tokens) {
    ...
    let balance = await tokenContract.balanceOf(strategyAgent598Address)
    if(balance.gt(0)) {
      console.log(`Agent has ${formatUnits(balance, decimals)} withdrawable ${symbol}`)
      txdatas.push(agentModule598.interface.encodeFunctionData("moduleD_sendBalanceTo", [boombotseth.address, address]))
    }
  }
  if(txdatas.length == 0) {
    ...
    let tx = await strategyAgent598.multicall(txdatas, {...networkSettings.overrides, gasLimit:txdatas.length*200_000})
    await watchTxForEvents(tx)
  }
}
...
async function getVaults() {
  // fetch data
  let url = `https://www.duo.exchange/api/getVaultPositions?owner=${strategyAgent598Address}&showClosed=0`
  let res: any
  try {
  ...
  console.log(s)
  console.log(`This agent can redeem ${formatUnits(amountRequired)} DUSD`)
}

async function transferIn() {
  if(amountRequired.eq(0)) {
    console.log(`No DUSD required, skipping`)
    return
  }
  let balA1 = await dusd.balanceOf(strategyAgent598Address)
  let diff = amountRequired.sub(balA1)
  if(diff.gt(0)) {
    let balU1 = await dusd.balanceOf(boombotseth.address)
    if(balU1.lt(diff)) {
      throw new Error(`The agent requires ${formatUnits(diff)} more DUSD but the user only has ${formatUnits(balU1)}. Get some more and come back`)
    }
    console.log(`Transferring DUSD from user to agent`)
    let tx = await dusd.transfer(strategyAgent598Address, diff, networkSettings.overrides)
    await watchTxForEvents(tx)
  }
  else {
    console.log(`No additional DUSD required, skipping`)
    return
  }
}
...
async function redeem() {
  if(amountRequired.eq(0)) {
    console.log(`No DUSD redeemable, skipping`)
    return
  }
  let balA1 = await dusd.balanceOf(strategyAgent598Address)
  if(balA1.lt(amountRequired)) {
    throw new Error(`The agent requires ${formatUnits(amountRequired)} DUSD but only has ${formatUnits(balA2)}. Check the status and retry`)
  }
  let itemType = 'pointOptimizeds'
  if(itemType == 'pointOptimizeds') { // if (state.mode == MODE.FIXED_RATE)
    let wrapMint = await agentModule598.wrapMint()
    let rateContract = await agentModule598.rateContract()
    let claimableOrbit = await agentModule598.callStatic.quoteClaim()
    console.log(`wrapMint        : ${wrapMint}`)
    console.log(`rateContract    : ${rateContract}`)
    console.log(`claimable orbit : ${formatUnits(claimableOrbit)}`)
    let txdata0 = agentModule598.interface.encodeFunctionData("moduleD_burnFixedRate", [wrapMint, rateContract, amountRequired])
    let txdata1 = agentModule598.interface.encodeFunctionData("moduleD_sendBalanceTo", [boombotseth.address, USDB_ADDRESS])
    //let txdatas = [txdata0, txdata1, txdata2]
    let txdatas = [txdata0, txdata1]
    if(claimableOrbit.gt(0)) {
      let txdata2 = agentModule598.interface.encodeFunctionData("moduleD_claimTo", [boombotseth.address])
      txdatas.push(txdata2)
    }
    console.log(`burning fixed rate and redeeming`)
    let tx = await strategyAgent598.multicall(txdatas, {...networkSettings.overrides, gasLimit: 1_000_000})
    await watchTxForEvents(tx)
  }
  else {
