...
// in order to make a call to Factory.blastooorMintWithAllowlist()
export function getMintFromAllowlistDigest(
    domainName: string,
    ...
    const DOMAIN_SEPARATOR = getDomainSeparator(domainName, factoryAddress, chainID)
    return utils.keccak256(
        utils.solidityPack(
        ["bytes1", "bytes1", "bytes32", "bytes32"],
        [
...
// in order to make a call to Factory.blastooorMintWithAllowlist()
export function getMintFromAllowlistSignature(
    domainName: string,
    factoryAddress: string,
    chainID: number,
    receiver: string,
  ...
  let digest = getMintFromAllowlistDigest(domainName, factoryAddress, chainID, receiver, typehash);
  //console.log('signer')
  //console.log(signer)
  //console.log(signer.privateKey)
  //console.log(signer.privateKey.slice(2))
  //console.log(Buffer.from(signer.privateKey.slice(2), "hex"))
  //console.log(sign(digest, Buffer.from(signer.privateKey.slice(2), "hex")))
  //console.log(assembleSignature(sign(digest, Buffer.from(signer.privateKey.slice(2), "hex"))))
  //let signature = assembleSignature(sign(digest, Buffer.from(signer.privateKey.slice(2), "hex")));
  let signature = assembleSignature(sign(digest, Buffer.from(signerKey, "hex")));
  return signature
}
